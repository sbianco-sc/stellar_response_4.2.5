    ...
    def check_tasks_status(task_result):
        ...
        for task in task_result.data["tasks"]:
            if task["status"] == "failed" or task["status"] == "partially succeeded":
                # Aella add err_message handling
                err_message = None
                try:
                    err_message = task["task-details"][0]["stagesInfo"][0]["messages"][0]["message"]
                except Exception as e:
                    err_message = task
                task_result.set_success_status(False, err_message=err_message)
                break
